...
#if 0 && defined(CONFIG_6D2)
//static struct semaphore *m2m_sem = NULL; // global to lazily avoid finding delete_semaphore() stub

extern void* edmac_copy_rectangle_cbr_start(void *dst, void *src,
                                     int src_width, int src_x, int src_y,
                                     int dst_width, int dst_x, int dst_y,
                                     int w, int h,
                                     void (*cbr_r)(void *), void (*cbr_w)(void *), void *cbr_ctx);
extern void edmac_copy_rectangle_adv_cleanup(void);

static void dummy_cbr(void *unused)
{
    return;
}

static void cbr_w(void *unused)
{
//    give_semaphore(m2m_sem);
    edmac_copy_rectangle_adv_cleanup();
}

static void release_mem_to_mem_sem(struct semaphore *sem)
{
    give_semaphore(sem);
}

static void delay_dumpf(void *unused)
{
    msleep(4000);
    uint32_t *probe = (uint32_t *)0x2a588;
    if (probe != NULL)
    {
        DryosDebugMsg(0, 15, "dprobe: %x", *probe);
        probe = (uint32_t *)(*probe);
        if (probe != NULL)
        {
            DryosDebugMsg(0, 15, "*dprobe: %x", *probe);
            DryosDebugMsg(0, 15, "m2m_resLock status: %d", *(probe + 1));
            DryosDebugMsg(0, 15, "m2m_resLock semaphore: %x", *(probe + 2));
        }
    }
    DryosDebugMsg(0, 15, "dumpfing");
    call("dumpf");
}
#endif
...
#if 0 && defined(CONFIG_6D2)
#include "log.h"
// time to log channel usage via reading shamem...

    struct dryos_chan
    {
        struct edmac_mmio *mmio;
        uint32_t mode_bits;
    };
    ...
    struct chan_info chans[0x34] = {0};

    // global array mapping channel index to MMIO addr (and mode bits)
    struct dryos_chan *dryos_chans = (struct dryos_chan *)0x5f05c;
    
    int32_t start = get_ms_clock();
    int32_t end = start + 20000;
    // record channel usage for some time
    while(get_ms_clock() < end)
    {
        for (int i = 0; i < 0x34; i++)
        {
            uint32_t ram_addr = shamem_read(((uint32_t)dryos_chans[i].mmio) + 0x8);
            if (ram_addr != 0)
            {
                chans[i].ever_used = 1;
                chans[i].ram_addr = ram_addr;
            }
        }
        msleep(40);
    }
    ...
    // dump info on channels
    static uint8_t *log_buf = NULL;
    log_buf = malloc(MIN_LOG_BUF_SIZE);
    if (log_buf == NULL)
        return;
    if (init_log(log_buf, MIN_LOG_BUF_SIZE, "chans.log") < 0)
        return;

    char buf[0x200];
    send_log_data_str("chan_index, ever_used, ram_addr\n");
    for (int i = 0; i < 0x34; i++)
    {
        snprintf(buf, 0x200, "%x, %x, %x\n",
                 i, chans[i].ever_used, chans[i].ram_addr);
        send_log_data_str(buf);
    }
#endif

#if 0 && defined(CONFIG_6D2)
    uint32_t height = 256, width = 256;
    uint32_t src_size = height * width;
    uint32_t dst_size = src_size;
    uint8_t *src = malloc_aligned(src_size + dst_size, 0x1000);
    uint8_t *dst = src + src_size;

    // create some initial content for our test copy
    //
    ...
    info_led_blink(5, 60, 60);
    task_create("delay_dumpf", 0x1d, 0x600, delay_dumpf, NULL);
    // function_overrides impl, should be blocking
    edmac_copy_rectangle_cbr_start(dst, src,
                width, 0, 0,
                width, 0, 0,
                width, height,
                &dummy_cbr, &cbr_w, NULL);

    DryosDebugMsg(0, 15, "Copy complete\n");

    // check copy worked correctly
    if (src[src_size - 1] == 0xfe
        && dst[0] == 0x78
        && dst[5] == 0x11
        && dst[dst_size - 1] == 0xfe)
    {
        DryosDebugMsg(0, 15, "Copy was correct!\n");
    }
    else
    {
        DryosDebugMsg(0, 15, "Copy was incorrect!\n");
    }
    ...
    #include "edmac.h"
    // Getting edmac memcpy to work.  6D2 doesn't exactly have
    // the convenience function that 200D does.  But it does have
    // what might be inlining of the same:
    //
    //  create_mem_to_mem_lock_and_pwrmng_globals(0x2f,0x15,&DAT_e0ed7674,3);
    //  set_mem_to_mem_cbr(0xe01480bd,0); // cbr sets flag bits
    //  lock_and_wake_mem_to_mem();
    //  mem_to_mem_setup_copy((edmac_info *)&DAT_e0ed7648,(edmac_info *)&DAT_e0ed7648);
    ...
    // Do we need to serialise this?  Untested.  For raw video,
    // might be okay, if we're not sharing read or write regions?
    //
    // They use flags, but we can probably use sems, which avoid
    // having to find event flag stubs.
    if (m2m_sem == NULL)
        m2m_sem = create_named_semaphore("m2m", SEM_CREATE_UNLOCKED);
    int err = take_semaphore(m2m_sem, 1000);
    if (err)
    {
        return;
    }

    // function_overrides impl.
    edmac_copy_rectangle_cbr_start(dst, src,
                256, 0, 0,
                256, 0, 0,
                256, 256,
    ...
    err = take_semaphore(m2m_sem, 1000);
    if (err)
    {
        DryosDebugMsg(0, 15, "Error taking m2m_sem after copy\n");
    }
    edmac_copy_rectangle_adv_cleanup();
    give_semaphore(m2m_sem);
    DryosDebugMsg(0, 15, "Copy complete\n");
    ...
    // This must come before take_mem_to_mem_lock(), to avoid an assert.
    // That occurs because the "pwrmng_stuff" assigns a pwrmng ID to a global
    // (twice, one for the src chan, one for dst chan).  These are checked
    // to be valid IDs by (take|give)_mem_to_mem_lock(), which maybe
    // use them to wake then sleep the devices.
    //
    // Probably should rename take_mem_to_mem_lock() -> wake_and_lock_mem_to_mem() ?
    //
// This works outside of LV, but not within (presumably bad channels or ResLock).
//    create_mem_to_mem_lock_and_pwrmng_stuff(0x2f, 0x15, (void *)0xe0ed7674, 3);

    // Dumping info via edmaclog.mo, 0x21 and 0x24 look like free read channels
    // during LV and video recording.
    // 0x13 looks like a plausible write channel, along with maybe 2 and 3
    // (those might be special, they're low numbers?)
// This partly works in stills LV; crashes on exit from LV.
// Video LV restarts cam after a pause (channels okay, bad reslock?)
//    create_mem_to_mem_lock_and_pwrmng_globals(0x21, 0x13, (void *)0xe0ed7674, 3);

    // Can we use the same trick as 200D, lock with some unrelated device?
    // Yes, or at least pretty well.  This works in LV or movie mode, but seems
    // to lock display if actively recording while you try it.  Doesn't crash though,
    // and the copy completes later on (when you stop recording?).
    // Suggests we're waiting for one of the locks.
    ...
    set_mem_to_mem_cbr(release_mem_to_mem_sem, m2m_sem);
    ...
    mem_to_mem_set_src_dst_and_start(src, dst);

    err = take_semaphore(m2m_sem, 1000);
    if (err)
    {
        printf("Error taking m2m_sem after copy\n");
    }
    give_semaphore(m2m_sem);
